...
## Stable identity for persistence (met_npcs in the save file).
@export var npc_id: String = ""
## villager | kid | vendor | elder | shopkeeper — flavor hook for later
## per-NPC memory; no behavior branches on it yet.
@export var npc_type: String = "villager"
@export var greeting: String = "Hey there! Nice day for a walk!"
@export var repeat_text: String = "Good to see you again!"
@export var sprite_texture: Texture2D

## Set from the save file by Main before the player can interact.
var already_met: bool = false
...
## Pure deterministic greet. First meeting: full intro + mood boost.
## Every later talk: short repeat line, no boost. Unit-testable.
func greet() -> Dictionary:
	if already_met:
		return {"text": repeat_text, "mood_boost": 0.0}
	already_met = true
	return {"text": greeting, "mood_boost": MOOD_BOOST}


## Runtime wrapper: emits greeted signal.
func try_greet() -> void:
	var result := greet()
